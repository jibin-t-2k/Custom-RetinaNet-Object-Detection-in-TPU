...
def resize_and_pad_image(image, min_side=256, max_side=384, jitter=None, stride=128.0):
...
def preprocess_data(sample):
    ...
    """
    image = sample["image"]
    bbox = swap_xy(sample["bboxs"])
    class_id = tf.cast(sample["category_ids"], dtype=tf.int32)
...
def train_aug_fn(image, bboxes, category_ids):
    data = {"image":image, 'bboxes': bboxes, 'category_ids': category_ids}
    ...
    category_ids = tf.cast(category_ids, tf.float32)
    return image, bboxes, category_ids

def preprocess_train_data(image, bboxes, category_ids):
    """Applies preprocessing step to a single train sample
    ...
    Returns:
      image: Resized and padded image with random horizontal flipping applied.
      bboxes: Bounding boxes with the shape `(num_objects, 4)` where each box is
        of the format `[x, y, width, height]`.
      class_id: An tensor representing the class id of the objects, having
        shape `(num_objects,)`.
    """
    ...
    image, bboxes, category_ids = tf.numpy_function(func=train_aug_fn, inp=[image, bboxes, category_ids], Tout=[tf.float32, tf.float32, tf.float32])
...
def preprocess_test_data(image, bboxs, category_ids):
    """Applies preprocessing step to a single test sample
    ...
    Returns:
      image: Resized and padded image with random horizontal flipping applied.
      bboxs: Bounding boxes with the shape `(num_objects, 4)` where each box is
        of the format `[x, y, width, height]`.
      class_id: An tensor representing the class id of the objects, having
        shape `(num_objects,)`.
    """
    ...
    # transformed = test_transform(image=image, bboxes=bboxs, category_ids=class_id)
...
val_dataset = tf.data.TFRecordDataset("test.tfrec")
...
ssd_history = model.fit(
                        ...
                        epochs=epochs,
                        steps_per_epoch = 2154 // batch_size,
                        validation_steps = 535 // batch_size,
...
test_dataset = tf.data.TFRecordDataset("/content/tf_data/test.tfrec")
